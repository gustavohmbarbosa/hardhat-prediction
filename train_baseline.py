...
MODEL       = "yolov8n.pt"     # pretrained nano (COCO)
IMGSZ       = 320
EPOCHS      = 60
BATCH       = 16
WORKERS     = 2
...
PROJECT     = "runs_train"
NAME        = "baseline_v8n_320"
DEVICE      = "cuda:0" if torch.cuda.is_available() else "cpu"
MEASURE     = True             # tenta reportar FLOPs/Params se thop estiver instalado
# --------------------------------------
...
def main():
    ...
    print(f"[train] model={MODEL} data={DATA_YAML} imgsz={IMGSZ} epochs={EPOCHS} batch={BATCH} device={DEVICE}")
    model = YOLO(MODEL)
    results = model.train(
        data=DATA_YAML,
        imgsz=IMGSZ,
